...
function buildSlotListEmbed(scrim) {
  const embed = new EmbedBuilder()
    .setTitle(`🏆 ${scrim.scrimName || 'BGMI Scrim'} — Slot List`)
  ...
  for (let i = 0; i < lines.length; i += chunkSize) {
    ...
    // Zero-width space as the field name instead of "Slots X-Y" — Discord
    // embed fields require a non-empty name, but this keeps the list
    // reading as one continuous 1-20 block with no visible section breaks.
