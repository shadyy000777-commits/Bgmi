...
async function handleRegisterTeamButton(interaction) {
  ...
  await interaction.reply({
    content: saved
      ? `You have a saved team profile: **${saved.team_name}**`
      : "You don't have a saved team profile yet — **Register New Team** to create one.",
    components: [row1, row2],
    flags: MessageFlags.Ephemeral,
  });
}

function buildRegistrationCompleteEmbed(data, slotNumber, scrimName) {
...
async function handleUseOldTeam(interaction) {
  ...
  await interaction.update({
    content: null,
    embeds: [buildRegistrationCompleteEmbed(saved, result.assigned, store.scrim.scrimName)],
    components: [],
  });
}
...
async function handleRegSelectPlayers(interaction) {
  ...
  await interaction.update({
    content: null,
    embeds: [buildRegistrationCompleteEmbed(data, result.assigned, store.scrim.scrimName)],
    components: [],
  });
}
