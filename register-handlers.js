...
async function handleUseOldTeam(interaction) {
  ...
  // to scrim even when the team profile itself hasn't. handleRegSelectPlayers
  // (the same handler the "Register New Team" flow ends on) takes it from here.
  const entry = startPending(interaction.user.id, interaction.guildId, 'use-old-team');
  Object.assign(entry.data, saved);

  await interaction.update({
    content: '✅ Using your saved team profile. Last step — select the **4 players from this server** who will play:',
    embeds: [],
