const {
  ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, UserSelectMenuBuilder,
} = require('discord.js');
...
async function handleRegisterTeamButton(interaction) {
  ...
  if (saved) {
    ...
    const row2 = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('register_new_team').setLabel('Register New Team').setStyle(ButtonStyle.Primary)
    );
  ...
  // No saved profile at all — nothing to reuse, so go straight to the form.
...
async function handleUseOldTeam(interaction) {
  ...
  await interaction.update({
    content: `✅ **${saved.team_name}** registered using your saved profile! You've been allotted **Slot ${result.assigned}**.`,
    components: [],
  });
}
...
async function handleRegStep3Submit(interaction) {
  ...
  const p5Match = P5_COMBINED_RE.exec(p5_combined);
  if (!p5Match) {
    return interaction.reply({
      content: '❌ Player 5 must be in the format **IGN - UID** (e.g. `ProGamer123 - 5123456789`). Tap **Try Again** to fix it — your other answers are kept.',
      components: [retryRow('reg_retry_step3', 'Try Again')],
      flags: MessageFlags.Ephemeral,
    });
  }
  const p5_ign = p5Match[1].trim();
  const p5_uid = p5Match[2].trim();
  ...
  const requiredFields = [
    ...
    'p1_ign', 'p1_uid', 'p2_ign', 'p2_uid', 'p3_ign', 'p3_uid', 'p4_ign', 'p4_uid', 'p5_ign', 'p5_uid',
  ];
...
async function handleRegSelectPlayers(interaction) {
  ...
  await interaction.update({
    content: `✅ **${data.team_name}** registered! You've been allotted **Slot ${result.assigned}**. Your team profile has also been saved for next time.`,
    components: [],
  });
}
