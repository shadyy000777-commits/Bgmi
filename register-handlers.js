...
async function handleUseOldTeamContinue(interaction) {
  ...
  if (options.length === 0) {
    return interaction.update({
      content: '❌ Sorry, every group is currently full — but your team profile has been saved for the next scrim.',
      embeds: [], components: [],
    });
  }
  ...
  await interaction.update({
    content: 'Pick the group (match time) you\'d like to play in:',
    embeds: [],
    components: [new ActionRowBuilder().addComponents(menu)],
  });
}
