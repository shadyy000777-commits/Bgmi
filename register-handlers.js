...
// Groups are tracked internally as letters (A-L, matching GROUP_SCHEDULE
// below and /set-group-role's storage keys) but shown to players as
// numbers — "Group A" displays as "Group 1", etc.
function groupDisplayName(letter) {
  return `Group ${letter.charCodeAt(0) - 64}`; // 'A' -> 1, 'B' -> 2, ...
}
...
function buildRegistrationLogEmbed(data, ownerId, slotNumber, group, scrimName, isSlotChange = false) {
  return new EmbedBuilder()
...
async function postRegistrationLog(interaction, store, data, ownerId, result, isSlotChange = false) {
  ...
  try {
    ...
    await logChannel.send({
      embeds: [buildRegistrationLogEmbed(data, ownerId, result.assigned, result.group, store.scrim.scrimName, isSlotChange)],
    });
  } catch (err) {
...
function listGroupsWithFreeSlots(scrim) {
  ...
  for (let index = 0; ; index++) {
    ...
    groups.push({ letter: String.fromCharCode(65 + index), freeCount });
  }
  return groups;
}
...
function groupTimeSummary(letter) {
  const schedule = GROUP_SCHEDULE[letter];
...
async function handleChangeSlotButton(interaction) {
  ...
  const options = listGroupsWithFreeSlots(scrim)
    ...
    .map(g => ({
      ...
      description: `${groupTimeSummary(g.letter)} — ${g.freeCount} slot(s) free`,
      value: g.letter,
    }));
...
async function handleChangeSlotGroupSelect(interaction) {
  ...
  if (!scrim || !scrim.open) {
    return interaction.update({ content: '❌ Registration is not open right now.', components: [] });
  }
  ...
  await interaction.update({
    content: null,
    embeds: [buildAlreadyRegisteredEmbed(teamData, scrim.scrimName)],
    components: [],
  });
}
...
// Gives the role configured for `newGroupLetter` via /set-group-role, and —
// if the player was previously in a different group with a different
// role configured — removes that old group's role first, so a player
// only ever holds one group role at a time. Never throws, same reasoning
// as giveRegisteredRole: a role hiccup should never block registration.
async function giveGroupRole(interaction, store, newGroupLetter, previousGroupLetter) {
  ...
  const newRoleId = groupRoles[newGroupLetter];
  const oldRoleId = previousGroupLetter ? groupRoles[previousGroupLetter] : null;

  if (!newRoleId) {
    console.warn(`[group-role] No role configured for Group ${newGroupLetter} in guild ${interaction.guildId} — run /set-group-role.`);
    return;
  }
  ...
  try {
    ...
    if (!newRole) {
      console.error(`[group-role] Configured role ${newRoleId} for Group ${newGroupLetter} no longer exists in guild ${interaction.guildId} — re-run /set-group-role.`);
      return;
    }
    if (newRole.position >= botMember.roles.highest.position) {
      console.error(`[group-role] Bot's highest role is below "${newRole.name}" (${newRoleId}) in guild ${interaction.guildId} — move the bot's role above it.`);
      return;
    }
