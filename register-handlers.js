...
const {
  ...
  isGroupClosed, nextOpenGroupLetters, dayBatchForLetter, dayLabelForBatch, resolveGroupSchedule,
...
// card below (dayNumber is days-since-epoch, same value dayLabelForBatch
// uses internally).
...
// detailed staff-facing card (team number, owner, full per-player IGN/UID
// breakdown, match date + schedule, substitute) rather than the plain grid
// shown to the player themselves. `profile` is always the player's full
// saved verification/registration profile (see postRegistrationLog below)
// so every field here — including Team Number and the individual player
// rows — is available whether this is a fresh registration or a slot change.
function buildRegistrationLogEmbed(profile, ownerId, slotNumber, group, scrimName, store, isSlotChange = false) {
...
async function postRegistrationLog(interaction, store, data, ownerId, result, isSlotChange = false) {
  ...
  try {
    await logChannel.send({
      embeds: [buildRegistrationLogEmbed(profile, ownerId, result.assigned, result.group, store.scrim.scrimName, store, isSlotChange)],
    });
  } catch (err) {
...
async function handleUseOldTeamContinue(interaction) {
  ...
  const letters = nextOpenGroupLetters(scrim);
