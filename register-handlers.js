...
const {
  ...
  isGroupClosed, currentDayBatchIndex, lettersForDayBatch, dayLabelForBatch,
...
function assignSlot(store, userId, data, preferredGroup) {
  ...
  if (preferredGroup) {
    ...
    for (let i = FIRST_ASSIGNABLE_SLOT; i <= scrim.totalSlots; i++) {
      ...
      // empty (see closeGroup / clearGroupRegistrations). Registration always
      // lands in the earliest group that's neither full nor closed.
      if (isGroupClosed(scrim, groupLetterForSlot(i))) continue;
      if (!scrim.slots[i]) { assigned = i; break; }
    }
    if (assigned === null) {
...
// Posts the registration summary to the channel configured via
// /set-registration-channel if one was set, otherwise the auto-created
// shared log channel — so this always logs somewhere with zero admin
// setup required. Never throws — a missing channel, missing permissions,
// or the channel being deleted shouldn't ever block the player's own
// confirmation.
async function postRegistrationLog(interaction, store, data, ownerId, result, isSlotChange = false) {
  const logChannel = await resolveLogChannel(interaction.guild, store, store.settings && store.settings.registrationLogChannelId);
    ...
    const channel = await interaction.guild.channels.create({
      ...
      permissionOverwrites: [
        { id: interaction.guild.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] },
        {
          ...
          // Locked by default — nobody in the group can type or attach
          // files until an admin runs !open (see pcmd-open.js).
...
async function handleUseOldTeamContinue(interaction) {
  ...
  // Only ever offer the current day-batch of (up to) 4 groups — matches the
  // "only 4 groups play per real day" schedule everything else (live panel,
  // match times) is built around, instead of listing every group that's
  // ever been opened. Groups 1-4 show until all 4 are full/closed, then it
  // flips to 5-8, then 9-12, etc. — see currentDayBatchIndex in
  // group-schedule.js for the exact advancement + calendar-day-cap rules.
  const batchIndex = currentDayBatchIndex(scrim);
  const batchLetters = lettersForDayBatch(scrim.totalSlots, batchIndex);
  const { relative, dateLabel } = dayLabelForBatch(scrim, batchIndex);

  const options = batchLetters
    .filter(letter => !isGroupClosed(scrim, letter))
    .map((letter) => {
  ...
  if (options.length === 0) {
    return interaction.update({
      content: `❌ Sorry, **${relative}**'s groups are currently full — but your team profile has been saved for the next scrim.`,
      embeds: [], components: [],
    });
  }
