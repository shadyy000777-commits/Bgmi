...
async function handleChangeSlotButton(interaction) {
  ...
  await interaction.reply({
    content: `You're currently in **Group ${already.group}** (Slot ${localSlotNumber(already.slotNumber)}). Pick a different group below to switch your match time:`,
...
async function handleChangeSlotGroupSelect(interaction) {
  ...
  if (already.group === newGroup) {
    return interaction.update({ content: `You're already in Group ${newGroup}.`, components: [] });
  }
  ...
  if (newSlotNumber === null) {
    return interaction.update({
      content: `❌ Group ${newGroup} just filled up — run **Change Slot** again to pick another group.`,
      components: [],
    });
  }
