...
// Standard BGMI/PUBGM placement point table (WWCD down to 1 kill's worth of
// value at the bottom) — servers can override any of this via "Set Points
// System" in the !pt panel, this is just a sane default so the feature
// works immediately with zero setup.
function defaultPointsSystem() {
  return {
    killPoints: 1,
    placements: {
...
function defaultPointTable() {
  return {
    pointsSystem: defaultPointsSystem(),
    design: { serverName: null, instagram: null, discordInvite: null, youtube: null, theme: 'navy' },
  };
}
...
//     pointTable: { pointsSystem: { killPoints, placements: { "1": 15, "2": 12, ... } },
//       design: { serverName, instagram, discordInvite, youtube, theme } }
//   }
// }
...
function getGuildStore(guildId) {
  ...
  if (all[guildId].pointTable === undefined) all[guildId].pointTable = defaultPointTable();
  if (all[guildId].pointTable.pointsSystem === undefined) all[guildId].pointTable.pointsSystem = defaultPointsSystem();
  if (all[guildId].pointTable.design === undefined) all[guildId].pointTable.design = { serverName: null, instagram: null, discordInvite: null, youtube: null, theme: 'navy' };
  if (all[guildId].pointTable.design.theme === undefined) all[guildId].pointTable.design.theme = 'navy';
  return all[guildId];
}
