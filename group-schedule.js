...
// The day-batch that is "live" right now — i.e. whichever batch's real-world
// match day it currently is, counting from the day the scrim was created
// (day-batch 0). Clamped so it never runs past the last batch this scrim's
// totalSlots can support.
...
// Whether there's a day-batch after the currently-live one (i.e. whether
// there's a "next day" to show on the live panel at all).
function hasNextDayBatch(scrim) {
  return currentDayBatchIndex(scrim) < totalDayBatches(scrim.totalSlots) - 1;
}

// The day-batch to actually display on the live panel — always the one
// AFTER whichever is currently live, since registration never closes but
// only one day's worth of groups plays at a time in real life.
function nextDayBatchIndex(scrim) {
  const maxIndex = totalDayBatches(scrim.totalSlots) - 1;
  return Math.min(currentDayBatchIndex(scrim) + 1, maxIndex);
}
