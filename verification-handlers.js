...
// Posted to the PUBLIC channel configured via /set-verify-channel — a short
// summary only (team, owner mention, city, playing lineup as Discord
// mentions). Deliberately doesn't include WhatsApp, email, or per-player
// IGN/UID — those are contact/roster details that only belong in the
// private card below.
function buildPublicVerifiedLogEmbed(data, teamNumber, ownerId, isEdit) {
  const lineupLine = (data.selectedPlayerIds || []).map(id => `<@${id}>`).join(' ') || '_not selected_';

  return new EmbedBuilder()
    .setTitle(isEdit ? '✏️ Team Updated' : '✅ Team Verified')
    .setColor(isEdit ? 0x5865F2 : 0x57F287)
    .setDescription(
      `🏆 ${teamNumber} : **TEAM ${data.team_name}**\n` +
      `👑 Owner - <@${ownerId}>\n` +
      `📍 City - ${data.city}\n\n` +
...
// — the full detail card (team number, owner mention, city, WhatsApp,
// email, per-player IGN/UID, playing lineup). Also reused by !team
...
async function handleVerifyPlayerSelect(interaction) {  const pendingEntry = getPending(interaction.user.id);
  ...
  if (logChannel) {
    try {
      await logChannel.send({ embeds: [buildPublicVerifiedLogEmbed(data, teamNumber, interaction.user.id, isEdit)] });
    } catch (err) {
  ...
  // Full detail card only goes to the private channel if one was
  // explicitly configured — no fallback, since silently dumping WhatsApp/
  // email/IGN-UID into a channel nobody meant to be private would defeat
  // the whole point.
  ...
  if (privateChannelId) {
    ...
    if (privateChannel) {
      try {
        await privateChannel.send({ embeds: [buildVerifiedLogEmbed(data, teamNumber, interaction.user.id, isEdit)] });
      } catch (err) {
