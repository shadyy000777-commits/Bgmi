    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('slots').setLabel('Total slots').setStyle(TextInputStyle.Short)
          .setPlaceholder('e.g. 25').setRequired(true).setMaxLength(3)
      ),
    );
}
...
function buildScrimEditModal(scrim) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('slots').setLabel('Total slots').setStyle(TextInputStyle.Short)
          .setValue(String(scrim.totalSlots)).setRequired(true).setMaxLength(3)
      ),
    );
}
...
async function handleScrimCreateModalSubmit(interaction) {
  ...
  if (!Number.isInteger(totalSlots) || totalSlots < 1 || totalSlots > 500) {
    return interaction.reply({ content: '❌ Total slots must be a whole number between 1 and 500 (25 groups of 20 — the max Discord group letters/select menus can support).', flags: MessageFlags.Ephemeral });
  }
