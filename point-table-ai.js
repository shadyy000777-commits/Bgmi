...
// Groq currently caps vision requests at 5 images per request, so lobby and
// result screenshots (which can be up to 5 + 10 = 15 total) are sent as
// separate requests in batches of this size and the results are merged.
...
async function callGroqVision(apiKey, systemPrompt, userText, imageUrls) {
  ...
  if (!text) throw new Error('Groq vision API response had no text.');
...
Rules:
- Treat each screenshot as its own separate match unless it's obviously a continuation (a second screenshot of the same table scrolled down) — in that case merge it into the same match.
...
async function readLobbyScreenshots(imageUrls, slotlistText) {
  ...
  for (const batch of batches) {
    let userText = 'Read the lobby slot list from these screenshot(s).';
    ...
    const parsed = await callGroqVision(apiKey, LOBBY_SYSTEM_PROMPT, userText, batch);
...
async function readResultScreenshots(imageUrls) {
  ...
  for (const batch of batches) {
    const userText = 'Read the match result table(s) from these screenshot(s). Treat each screenshot as a separate match unless stated otherwise.';
    const parsed = await callGroqVision(apiKey, RESULT_SYSTEM_PROMPT, userText, batch);
