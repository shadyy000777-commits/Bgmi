...
Read carefully — zoom your attention to each row individually rather than skimming the whole image at once. Slot numbers and names are small; do not guess a digit or letter you can't clearly make out.
...
Read the table row by row, left to right, exactly as printed. Placement and kill counts are numeric digits — read each digit individually rather than pattern-matching a "typical" value. Do not round, estimate, or infer a number you can't clearly see; if a specific cell is genuinely illegible, use null for that field rather than guessing.

After reading every row, sanity-check yourself before answering: placements in a single match should be distinct numbers starting at 1 with no duplicates or gaps you can't account for. If your reading would produce a duplicate or missing placement, re-examine that row instead of leaving the inconsistency in your answer.

Respond with ONLY a JSON object, no prose, in this exact shape:
{ "matches": [ { "map": "<string or null>", "results": [ { "placement": <number or null>, "teamName": "<string>", "kills": <number or null> } ] } ] }
