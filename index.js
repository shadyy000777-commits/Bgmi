...
// "show me players with the Scrims Ban role" and answers them straight from
...
//
// Role is resolved two ways: an actual @role mention (most reliable), or a
// plain-text role name found in the message (matched longest-name-first so
// e.g. "Group A" doesn't accidentally match a broader role like "Group").
// Returns true if it handled (and replied to) the message, false if this
// wasn't a role-list request at all — so the caller falls through to the AI.
async function tryHandleRoleListRequest(message, cleanContent) {
  ...
  if (!role) {
    ...
    role = roles.find(r => lower.includes(r.name.toLowerCase())) || null;
  }
  if (!role) return false; // looked like a list request, but no role we could pin down — let the AI handle/clarify
  ...
  if (all.length === 0) {
    await message.reply(`No one currently has the **${role.name}** role.`);
    return true;
  }

  // Random sample of up to 10 each time there are more members than that,
  // so asking again doesn't always return the identical list.
  const picked = all.length <= 10 ? all : all.sort(() => Math.random() - 0.5).slice(0, 10);
  const lines = picked.map(m => `• ${m}`).join('\n');
  await message.reply(`**${role.name}** — showing ${picked.length} of ${all.length}:\n${lines}`);
  return true;
}
