...
async function clearGroupRegistrations(interaction, store, groupLetter) {
  const scrim = store.scrim;
  const slotEntries = scrim ? Object.entries(scrim.slots).filter(([, slot]) => slot.group === groupLetter) : [];
  if (!slotEntries.length) return { cleared: 0, failed: [] };
  ...
  for (const [slotKey, slot] of slotEntries) {
    ...
    delete scrim.slots[slotKey];
  }

  return { cleared: slotEntries.length, failed };
}
...
async function handleGroupAdminResultModalSubmit(interaction) {
  ...
  // an admin deletes this group's channel/role.
  closeGroup(scrim, groupLetter);
  const clearResult = await clearGroupRegistrations(interaction, store, groupLetter);
  ...
  // Also close the group's chat back down so only admins can post there
  // until the next time it's opened (see pcmd-open.js).
  const channelId = store.settings.groupChannels && store.settings.groupChannels[groupLetter];
