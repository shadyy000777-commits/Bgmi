...
async function handleGroupAdminButton(interaction) {
  ...
  switch (rawAction) {
    ...
    case 'group_admin_delete': {
      ...
      // Build what will be deleted
      const toDelete = [];
      if (channelId) toDelete.push('**Channel**');
      if (roleId) toDelete.push('**Role**');
      ...
      return interaction.reply({
        content: `⚠️ **Delete ${groupDisplayName(groupLetter)}?**\n\nThis will permanently delete:\n${toDelete.map(x => `• ${x}`).join('\n')}\n\n❌ This can't be undone. Registered teams stay on record, but the channel and role won't come back.`,
        components: [row],
        flags: MessageFlags.Ephemeral,
      });
    }
...
async function handleGroupAdminResultModalSubmit(interaction) {
  ...
  // 🆕 AUTO-CLEAR REGISTRATIONS FOR THIS GROUP
  // Remove all teams registered in this group's slots without kicking them from the group
  const { start, end } = slotRangeForGroup(groupLetter, scrim.totalSlots);
  let clearedCount = 0;
  for (let i = start; i <= end; i++) {
    if (scrim.slots[i]) {
      delete scrim.slots[i];
      clearedCount++;
    }
  }
  
  if (clearedCount > 0) {
    lines.push(`🧹 **Auto-cleared ${clearedCount} registration(s)** from ${groupDisplayName(groupLetter)} — players remain in the group.`);
    saveGuildStore(interaction.guildId, store); // Save the cleared slots
  }
