...
// Tell @discordjs/voice's internal transcoder exactly where ffmpeg lives.
// Railway's container doesn't have a system `ffmpeg` binary installed, and
// without this, any track that needs transcoding (not already Opus-encoded)
// fails silently — the player just fires an 'error' event and moves on,
// with nothing ever actually reaching the voice channel. Must run before
// any audio resource is created, so it's set here at module load.
...
async function addAndPlay({ guildId, voiceChannel, textChannel, url, requestedBy, adapterCreator }) {
  ...
  if (!queue) {
    ...
    player.on('error', (err) => {
      // Previously only logged to console — on Railway nobody's watching
      // that, so a playback failure (e.g. missing ffmpeg, a dead stream)
      // looked exactly like "nothing happened," with no indication why.
      console.error('[music-manager] Audio player error:', err);
      const failed = queue.songs[0];
      queue.textChannel?.send(
        `⚠️ Playback error on **${failed ? failed.title : 'current track'}** — skipping.\n` +
        `\`${err.message || err}\``
      ).catch(() => {});
      queue.songs.shift();
      playNext(guildId);
