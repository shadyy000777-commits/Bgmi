...
// Minimal containers (like Railway's) often ship with zero fonts installed,
// which makes canvas silently draw NO text at all (shapes/rects still work
// fine, since those don't need a font) — that's what caused tables to come
// back with colored bars but no team names or numbers. nixpacks.toml
// installs fonts-dejavu-core at build time; this double-checks common
// locations and registers it explicitly so "DejaVu Sans" always resolves
// instead of silently failing again if the package ever goes missing.
const DEJAVU_PATHS = [
...
for (const p of DEJAVU_PATHS) {
  try {
    if (fs.existsSync(p)) { GlobalFonts.registerFromPath(p, 'DejaVu Sans'); break; }
  } catch { /* best-effort — falls back to fontconfig resolution */ }
}
...
function renderPointTableImage(rows, design, themeKey) {
  ...
  ctx.font = 'bold 54px "DejaVu Sans", sans-serif';
