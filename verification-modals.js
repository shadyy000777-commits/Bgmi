...
function buildVerifyStep3Modal(prefill) {
  ...
  // Player 5 combined into one field: "IGN - UID". Prefill priority: an
  // in-progress p5_combined value (e.g. after a validation retry) beats
  // reconstructing it from a previously-saved p5_ign/p5_uid pair (used when
  // editing an existing verification, which was saved with them split).
  ...
  const p5combined = new TextInputBuilder()
    .setCustomId('p5_combined').setLabel('Player 5 - In-Game Name & UID')
    .setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(80)
    .setPlaceholder('e.g. ProGamer123 - 5123456789');
