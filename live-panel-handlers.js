...
function buildGroupRosterEmbed(store, groupLetter) {
  ...
  // Monospace table: Slot | Team | Owner. Owner is the plain name text
  // typed into the registration form (ownerName) — Discord doesn't render
  // real mentions inside a ``` code block (they just show as literal
  // text), so a genuine ping and this aligned table layout are mutually
  // exclusive; plain text was the choice made here.
  // Columns kept narrow — embed field values cap at 1024 chars, and a
  // full 20-row group would blow past that with wider columns.
  const colWidths = { slot: 4, team: 16, owner: 16 };
  const pad = (str, width) => {
    const s = String(str);
  ...
  for (let i = start; i <= end; i++) {
    ...
    const owner = slot ? (slot.ownerName || 'Unknown') : '-';
