...
function buildGroupRosterEmbed(store, groupLetter) {
  ...
  // Capped so the table stays readable on a narrow phone screen instead
  // of wrapping mid-row.
  const truncate = (str, max) => (str.length > max ? str.slice(0, max - 1) + '…' : str);
  const TEAM_WIDTH = 14;
  const OWNER_WIDTH = 12;

  const rows = [];
  for (let i = start; i <= end; i++) {
    const slot = frozenRoster ? frozenRoster[i] : scrim.slots[i];
    rows.push({
      slot: String(localSlotNumber(i)).padStart(2, '0'),
      team: truncate(slot ? slot.team : '-', TEAM_WIDTH),
      owner: truncate(slot ? (slot.ownerName || 'Unknown') : '-', OWNER_WIDTH),
    });
  }

  // A real bordered ASCII table — this only lines up in a monospace code
  // block, which means owner mentions can't resolve as live pings here
  // (they'd show as raw <@id> text), so the owner's name is printed
  // instead.
  const header = `Slot | ${'Team'.padEnd(TEAM_WIDTH)} | Owner`;
  const divider = '-'.repeat(header.length);
  const lines = rows.map(r => `${r.slot}   | ${r.team.padEnd(TEAM_WIDTH)} | ${r.owner.padEnd(OWNER_WIDTH)}`);
  const table = '```\n' + [header, divider, ...lines].join('\n') + '\n```';

  return new EmbedBuilder()
    .setColor(0x5865F2)
    .addFields({ name: `SLOTS ${localSlotNumber(start)}-${localSlotNumber(end)}`, value: table });
}
