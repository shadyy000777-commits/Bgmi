...
function buildGroupRosterEmbed(store, groupLetter) {
  ...
  // NOTE: Discord does not render mentions inside a ``` code block — a
  // real <@userId> in there just shows as literal text and never pings.
  // Since the Owner column needs to be an actual mention, this can't be a
  // monospace table anymore; it's back to plain lines (won't line up in
  // neat columns, but the mention will render/ping correctly).
  // Team names truncated at 14 chars, no bold markup on the slot number —
  // together these keep a full 20-slot group (long team names + full
  // <@id> mentions) safely under Discord's 1024-char embed field limit.
  const truncTeam = (str) => {
    const s = String(str);
  ...
  const rows = [];
  rows.push('Slot | Team | Owner');
  for (let i = start; i <= end; i++) {
    const slot = frozenRoster ? frozenRoster[i] : scrim.slots[i];
    const num = String(localSlotNumber(i)).padStart(2, '0');
    const team = slot ? truncTeam(slot.team) : '-';
    const owner = slot ? (slot.userId ? `<@${slot.userId}>` : (slot.ownerName || 'Unknown')) : '-';
    rows.push(`${num} | ${team} | ${owner}`);
  }
