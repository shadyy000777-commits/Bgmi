...
function buildGroupRosterEmbed(store, groupLetter) {
  ...
  for (let i = start; i <= end; i++) {
    ...
    // Plain text (not a code block) so the bold label renders and the
    // owner shows up as a real clickable/highlighted mention rather than
    // inert text.
    lines.push(slot ? `${label} ${slot.team}${slot.userId ? ` — <@${slot.userId}>` : ''}` : `${label} _empty_`);
  }
