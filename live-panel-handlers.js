...
const { ensureGroupAdminPanel } = require('./group-admin-panel');
...
// Re-renders (or, the first time, posts) the standing header + roster
// messages in a group's own channel, then makes sure the admin button panel
// is posted below them. Call this any time that group's roster changes — a
// fresh registration into it, a slot-change into or out of it, etc.
...
// message if an old one was deleted.
async function refreshGroupSlotList(client, guildId, groupLetter) {
  ...
  try {
    ...
    await upsertGroupMessage(channel, store, guildId, groupLetter, {
      ...
      build: () => ({ embeds: [buildGroupHeaderEmbed(store, groupLetter)] }),
    });
    ...
    await ensureGroupAdminPanel(client, guildId, groupLetter);
  } catch (err) {
...
module.exports = {
  ...
  buildGroupHeaderEmbed, buildGroupRosterEmbed, refreshGroupSlotList,
};
