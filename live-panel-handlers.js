...
function buildGroupRosterEmbed(store, groupLetter) {
  ...
  const embed = new EmbedBuilder()
    .setColor(0x5865F2)
    .setTitle(`SLOTS ${localSlotNumber(start)}-${localSlotNumber(end)}`);

  // Inline fields (not a code block) — Discord lays these out in grid
  // columns, giving a table-like look while still letting the owner
  // render as a real, clickable/highlighted @mention. Mentions never
  // resolve inside a ``` code block, so that route can't do both.
  for (let i = start; i <= end; i++) {
    ...
    if (slot) {
      ...
      embed.addFields({ name: `Slot ${num}`, value: `**${slot.team}**\n${owner}`, inline: true });
    } else {
      embed.addFields({ name: `Slot ${num}`, value: '*empty*', inline: true });
    }
  }
